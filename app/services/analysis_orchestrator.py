...
class AnalysisOrchestrator:
                    ...
                    try:
                        ...
                        if not default:
                            con_score = await self._generate_confidence_score(
                                statement=claim_text, analysis=analysis_content
                            )
    ...
    async def _generate_confidence_score(self, statement: str, analysis: str):
        messages = [
            LLMMessage(
                role="user",
                content=AnalysisPrompt.GET_CONFIDENCE.format(statement=statement, analysis=analysis),
            )
        ]
        response = await self._llm.generate_response(messages)
        text = response.text
